...
export function AccountCard({ data }: AccountCardProps) {
  const { areValuesVisible, openEditAccountModal, t, selectedMentoradoId } = useDashboard();
  ...
  return (
    <div
      className={cn(
        "p-4 bg-white rounded-2xl h-[200px] flex flex-col justify-between border-b-4 border-teal-950",
        !isReadOnly && "cursor-pointer"
      )}
      style={{ borderColor: color }}
      role={!isReadOnly ? "button" : undefined}
      onClick={!isReadOnly ? () => openEditAccountModal(data) : undefined}
    >
      <div>
