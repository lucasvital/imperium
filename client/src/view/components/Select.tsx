...
interface SelectProps {
  ...
  options: { value: string; label: string; category?: Category; disabled?: boolean }[];
            ...
            <RdxSelect.Content className="overflow-hidden z-[999] bg-white rounded-2xl border border-gray-100 shadow-[0px_11px_20px_0px_rgba(0,0,0,0.10)]">
              ...
              <RdxSelect.Viewport className="p-2 relative">
                {options.map((option) => {
                  ...
                  if (option.disabled) {
                    return (
                      <div key={option.value} className="px-2 py-1 text-xs font-semibold text-gray-500 dark:text-gray-400 pointer-events-none">
                        {option.label}
                      </div>
                    );
                  }
